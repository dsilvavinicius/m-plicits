...
def main():
    parser = argparse.ArgumentParser(
        description="Activate a conda environment, run an iNGP script, then generate mesh comparisons and compute distance metrics."
    )
    parser.add_argument(
        "conda_env",
    ...
    parser.add_argument("--steps", required=True, help="Number of steps.")
    parser.add_argument("--batch", required=True, help="Batch size.")
    parser.add_argument("--res", required=True, help="Resolution.")
    parser.add_argument(
        "--ingp_coarse_config", required=True, help="Path to iNGP coarse config JSON."
    )
    parser.add_argument(
        "--ingp_fine_config", required=True, help="Path to iNGP fine config JSON."
    )
    ...
    # ------------------------------------------------------------------
    # 1. Run the iNGP reconstruction inside the specified conda env
    # ------------------------------------------------------------------
    command = [
        "conda",
        "run",
        ...
        args.res,
        "--ingp_coarse_config",
        args.ingp_coarse_config,
        "--ingp_fine_config",
        args.ingp_fine_config,
    ]
    ...
    #     print(f"\nERROR: iNGP script exited with return code {result.returncode}")
    ...
    generate_pairs(
        ...
        output=json_file,
        suffixes=["_ingp_coarse", "_ingp_fine"]
    )
