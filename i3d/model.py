...
class SIREN(nn.Module):
    ...
    def from_pretrained_initial_condition(self, other: OrderedDict):
        ...
        keys = list(my_sd.keys())
        hh, hw = other[keys[0]].shape
        my_sd[keys[0]][:hh, :hw] = other[keys[0]]

        hh, hw = other[keys[-2]].shape
        my_sd[keys[-2]][:hh, :hw] = other[keys[-2]]

        ll = other[keys[1]].shape[0]
        my_sd[keys[1]][:ll] = other[keys[1]]
...
if __name__ == "__main__":
    ...
    for k, v in m2.state_dict().items():
        print(k, v)
    print("------------- m1 --------------")
    m.from_pretrained_initial_condition(m2.state_dict())
    for k, v in m.state_dict().items():
        print(k, v)
