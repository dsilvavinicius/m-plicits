...
def _curvature_segmentation(
    ...
    med_curvature_pts = vertices[
        (curvatures >= bin_edges[1]) & (curvatures < bin_edges[2]), ...
    ]
    ...
    low_idx = torch.randperm(low_curvature_pts.shape[0], device=device)[
        :n_low_curvature
    ]
    med_idx = torch.randperm(med_curvature_pts.shape[0], device=device)[
        :n_med_curvature
    ]
    high_idx = torch.randperm(high_curvature_pts.shape[0], device=device)[
        :n_high_curvature
    ]
...
def _read_ply(path: str, with_curvatures: bool = False):
    ...
    if with_curvatures:
        mesh.vertex["curvatures"] = o3c.Tensor(vertices[:, -2], dtype=o3c.float32)
...
def _create_training_data(
    ...
    if use_curvature:
        ...
        surf_pts, _ = _sample_on_surface(vertices, n_on_surf, device=device)

    coord_dict = {"on_surf": [surf_pts[..., :3], surf_pts[..., 3:6], surf_pts[..., -1]]}

    if n_off_surf > 0:
        domain_pts = np.random.uniform(
            domain_bounds[0], domain_bounds[1], (n_off_surf, 3)
        )
        ...
        domain_pts = domain_pts.to(device)
        coord_dict["off_surf"] = [
            domain_pts,
            torch.zeros_like(domain_pts, device=device),
            domain_sdf.squeeze(-1),
        ]
...
def _calc_curvature_bins(curvatures: torch.Tensor, percentiles: list) -> list:
    ...
    """
    try:
        q = torch.quantile(curvatures, torch.Tensor(percentiles).to(curvatures.device))
    except RuntimeError:
        ...
        q = torch.quantile(curvatures, torch.Tensor(percs).to(curvatures.device))
...
class PointCloud(Dataset):
    ...
    def __init__(
        ...
        try:
            mesh, vertices = _read_ply(mesh_path, with_curvatures=use_curvature)
        except:
            mesh = (
        ...
        if not batch_size or batch_size > (2 * self.vertices.shape[0]):
        ...
        if use_curvature:
            self.curvature_bins = _calc_curvature_bins(
                self.vertices[:, -2], curvature_percentiles
            )
...
if __name__ == "__main__":
    p = PointCloud(
        ...
        batch_size=10,
        use_curvature=True,
        curvature_fractions=(0.2, 0.7, 0.1),
    ...
    p = PointCloudDeferredSampling(
        "data/armadillo.ply", batch_size=10, use_curvature=False
    )
    print(len(p))
    print(p[0])
    print(p[0])
