...
def mean_curvature_test(grad, hess, x):
    grad_norm = torch.norm(grad, dim=-1)
    unit_grad = grad.squeeze(-1)/grad_norm.unsqueeze(-1)
    ...
    fx = grad[..., 0].unsqueeze(-1)
    fy = grad[..., 1].unsqueeze(-1)
    fz = grad[..., 2].unsqueeze(-1)

    Km_aux = fxx*(fy**2+fz**2)+fyy*(fx**2+fz**2)+fzz*(fy**2+fy**2) - 2*fx*fy*fxy- 2*fx*fz*fxz - 2*fy*fz*fyz
    km_aux = Km_aux/(2*grad_norm**3)
...
def discrete_laplacian(y, delta=1.0):
    """Get the discrete Laplacian of matrix M"""
    L = -4*y.clone()

    L += torch.roll(y, (0,-1), (0,1)) # right neighbor
    L += torch.roll(y, (0,+1), (0,1)) # left neighbor
    L += torch.roll(y, (-1,0), (0,1)) # top neighbor
    L += torch.roll(y, (+1,0), (0,1)) # bottom neighbor
