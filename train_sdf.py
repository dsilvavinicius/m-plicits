...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[],
        help="SIREN neurons per layer. By default we fetch it from the configuration file.",
    )
    parser.add_argument(
        "--batchsize",
        "-b",
    ...
    parser.add_argument(
        ...
        type=str,
        default="uniform",
        help='Uniform ("uniform", default value) or curvature-based ("curvature") sampling.',
    )
    parser.add_argument(
        "--curvature-fractions",
        type=float,
    ...
    parser.add_argument("--seed", type=int, default=0, help="RNG seed to use.")
    ...
    if withcurvature:
        ...
        if args.curvature_percentiles:
            curvature_percentiles = [float(p) for p in args.curvature_percentiles]
    ...
    # Fetch batch_size again in case it was zero originally
    ...
    for step in range(nsteps):
        # Recalculate the SDF points every resample_sdf_nsteps steps
    ...
    # Compute the file size of the best model file in bytes then convert to kilobytes
    ...
    with open(metrics_csv, "w", newline="") as csvfile:
        ...
        # Write header row
        csv_writer.writerow(
            ["Training Time (s)", "Num Parameters", "Model File Size (KB)"]
        )
